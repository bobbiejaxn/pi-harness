 ...
 * Extracted from slash-commands.ts.
 */
...
export function loadSavedOutputSchema(chain: ChainConfig, stepAgent: string, outputSchema: unknown): JsonSchemaObject | undefined {
...
export function extractSlashMessageText(content: string | Array<{ type?: string; text?: string }>): string {
...
export function formatExportPathList(paths: string[]): string {
	return paths.map((file) => `- \`${file}\``).join("\n");
}

export function collectResultPaths(results: SingleResult[], getPath: (result: SingleResult) => string | undefined): string[] {
	return results
		.map(getPath)
		.filter((file): file is string => typeof file === "string" && file.length > 0);
}

export function buildSlashExportText(response: SlashSubagentResponse): string {
...
export function persistSlashSessionSnapshot(ctx: ExtensionContext): void {
	try {
